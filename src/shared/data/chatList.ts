import { Chat } from '../type/chat'

export const CHAT_LIST: Chat[] = [
  {
    chatId: 759044099,
    title: 'Jo Jo',
    type: 'USER',
    filters: [],
    avatar: null,
    lastMessage: 'aboba'
  },
  {
    chatId: 1002118607,
    title: 'ЛИНИЯ ФРОНТА',
    type: 'CHANNEL',
    filters: [
      { id: 1, name: 'aboba', color: '#FFEA00', value: 'asdfdsfqfdsq', summary: false },
      { id: 2, name: 'aboba', color: '#2BFF00', value: 'asdfdsfqfdsq', summary: true }
    ],
    avatar: null,
    lastMessage: 'aboba'
  },
  {
    chatId: 1001166672,
    title: 'Москва сейчас',
    type: 'CHANNEL',
    filters: [],
    avatar: null,
    lastMessage: 'aboba'
  },
  {
    chatId: 1001849391,
    title: 'Общежитие №3',
    type: 'CHAT',
    filters: [
      { id: 1, name: 'Filter 1', color: '#00D9FF', value: 'Text for filter 1', summary: false },
      { id: 2, name: 'Filter 2', color: '#0048FF', value: 'Text for filter 2', summary: true },
      { id: 3, name: 'Filter 3', color: '#8800FF', value: 'Text for filter 3', summary: false },
      { id: 4, name: 'Filter 4', color: '#FF00DD', value: 'Text for filter 4', summary: true }
    ],
    avatar: null,
    lastMessage: 'aboba'
  },
  {
    chatId: 428317541,
    title: 'Джуси пуси на тусе',
    type: 'CHAT',
    filters: [
      { id: 1, name: 'Filter 1', color: '#FF0000', value: 'Text for filter 1', summary: false }
    ],
    avatar: null,
    lastMessage: 'aboba'
  },
  {
    chatId: 123456789,
    title: 'Tech News',
    type: 'CHANNEL',
    filters: [
      { id: 1, name: 'Filter 1', color: '#FF9500', value: 'Text for filter 1', summary: true },
      { id: 2, name: 'Filter 2', color: '#FFEA00', value: 'Text for filter 2', summary: false },
      { id: 3, name: 'Filter 3', color: '#2BFF00', value: 'Text for filter 3', summary: true },
      { id: 4, name: 'Filter 4', color: '#00D9FF', value: 'Text for filter 4', summary: false },
      { id: 5, name: 'Filter 5', color: '#0048FF', value: 'Text for filter 5', summary: true }
    ],
    avatar: null,
    lastMessage: 'aboba'
  },
  {
    chatId: 987654321,
    title: 'Анна К.',
    type: 'USER',
    filters: [
      { id: 1, name: 'Filter 1', color: '#8800FF', value: 'Text for filter 1', summary: true }
    ],
    avatar: null,
    lastMessage: 'aboba'
  },
  {
    chatId: 555666777,
    title: 'Фитнес клуб',
    type: 'CHAT',
    filters: [
      { id: 1, name: 'Filter 1', color: '#FF00DD', value: 'Text for filter 1', summary: true },
      { id: 2, name: 'Filter 2', color: '#FF0000', value: 'Text for filter 2', summary: false },
      { id: 3, name: 'Filter 3', color: '#FF9500', value: 'Text for filter 3', summary: true }
    ],
    avatar: null,
    lastMessage: 'aboba'
  },
  {
    chatId: 111222333,
    title: 'Кинообзоры',
    type: 'CHANNEL',
    filters: [
      { id: 1, name: 'Filter 1', color: '#FFEA00', value: 'Text for filter 1', summary: false },
      { id: 2, name: 'Filter 2', color: '#2BFF00', value: 'Text for filter 2', summary: true }
    ],
    avatar: null,
    lastMessage: 'aboba'
  },
  {
    chatId: 444555666,
    title: 'Иван Петров',
    type: 'USER',
    filters: [
      { id: 1, name: 'Filter 1', color: '#00D9FF', value: 'Text for filter 1', summary: true },
      { id: 2, name: 'Filter 2', color: '#0048FF', value: 'Text for filter 2', summary: false }
    ],
    avatar: null,
    lastMessage: 'aboba'
  },
  {
    chatId: 777888999,
    title: 'Автомобили',
    type: 'CHAT',
    filters: [
      { id: 1, name: 'Filter 1', color: '#8800FF', value: 'Text for filter 1', summary: true },
      { id: 2, name: 'Filter 2', color: '#FF00DD', value: 'Text for filter 2', summary: false },
      { id: 3, name: 'Filter 3', color: '#FF0000', value: 'Text for filter 3', summary: true }
    ],
    avatar: null,
    lastMessage: 'aboba'
  },
  {
    chatId: 222333444,
    title: 'Путешествия',
    type: 'CHANNEL',
    filters: [
      { id: 1, name: 'Filter 1', color: '#FF9500', value: 'Text for filter 1', summary: false }
    ],
    avatar: null,
    lastMessage: 'aboba'
  },
  {
    chatId: 888999000,
    title: 'Ольга С.',
    type: 'USER',
    filters: [
      { id: 1, name: 'Filter 1', color: '#FFEA00', value: 'Text for filter 1', summary: false },
      { id: 2, name: 'Filter 2', color: '#2BFF00', value: 'Text for filter 2', summary: true }
    ],
    avatar: null,
    lastMessage: 'aboba'
  }
]
